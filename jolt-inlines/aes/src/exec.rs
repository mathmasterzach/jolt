//! Core AES-128 round function implementation

// AES S-box substitution table
const SBOX: [u8; 256] = [
    ,
];

// Multiplication tables for MixColumns
const GMUL_2: [u8; 256] = [
    ,
];

const GMUL_3: [u8; 256] = [
    ,
];

/// SubBytes transformation - substitute each byte using the S-box
#[inline]
fn sub_bytes(state: &mut [u8; 16]) {
    for byte in state.iter_mut() {
        *byte = SBOX[*byte as usize];
    }
}

/// ShiftRows transformation
#[inline]
fn shift_rows(state: &mut [u8; 16]) {
    // Arrange bytes in 4x4 grid for clarity:
    // [ 0  1  2  3]      [ 0  1  2  3]
    // [ 4  5  6  7]  ->  [ 5  6  7  4]
    // [ 8  9 10 11]  ->  [10 11  8  9]
    // [12 13 14 15]  ->  [15 12 13 14]

    let temp = [
        state[0], state[5], state[10], state[15], state[4], state[9], state[14], state[3],
        state[8], state[13], state[2], state[7], state[12], state[1], state[6], state[11],
    ];

    state.copy_from_slice(&temp);
}

/// MixColumns transformation - multiply each column with the fixed polynomial
#[inline]
fn mix_columns(state: &mut [u8; 16]) {
    // Process each 4-byte column
    for col in 0..4 {
        let idx = col * 4;
        let s0 = state[idx];
        let s1 = state[idx + 1];
        let s2 = state[idx + 2];
        let s3 = state[idx + 3];

        state[idx] = GMUL_2[s0 as usize] ^ GMUL_3[s1 as usize] ^ s2 ^ s3;
        state[idx + 1] = s0 ^ GMUL_2[s1 as usize] ^ GMUL_3[s2 as usize] ^ s3;
        state[idx + 2] = s0 ^ s1 ^ GMUL_2[s2 as usize] ^ GMUL_3[s3 as usize];
        state[idx + 3] = GMUL_3[s0 as usize] ^ s1 ^ s2 ^ GMUL_2[s3 as usize];
    }
}

/// AddRoundKey transformation - XOR state with round key
#[inline]
fn add_round_key(state: &mut [u8; 16], round_key: &[u8; 16]) {
    for i in 0..16 {
        state[i] ^= round_key[i];
    }
}

/// Execute a single AES-128 round
/// Applies: SubBytes -> ShiftRows -> MixColumns -> AddRoundKey
pub fn execute_aes_round(mut state: [u8; 16], round_key: [u8; 16]) -> [u8; 16] {
    sub_bytes(&mut state);
    shift_rows(&mut state);
    mix_columns(&mut state);
    add_round_key(&mut state, &round_key);
    state
}

/// Execute the final AES-128 round (no MixColumns)
/// Applies: SubBytes -> ShiftRows -> AddRoundKey
pub fn execute_aes_final_round(mut state: [u8; 16], round_key: [u8; 16]) -> [u8; 16] {
    sub_bytes(&mut state);
    shift_rows(&mut state);
    add_round_key(&mut state, &round_key);
    state
}

/// Execute initial AddRoundKey (before any round)
pub fn execute_aes_add_round_key_initial(mut state: [u8; 16], round_key: [u8; 16]) -> [u8; 16] {
    add_round_key(&mut state, &round_key);
    state
}

//
#[inline]
pub fn execute_sbox8w(x: u64) -> u64 {
    let mut y = 0u64;
    for i in 0..8 {
        let byte = ((x >> (i * 8)) & 0xFF) as u8;
        y |= (SBOX[byte as usize] as u64) << (i * 8);
    }
    y
}
